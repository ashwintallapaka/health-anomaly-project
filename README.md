...
| `infra/spark-streaming.service` | systemd unit that keeps the streaming job running and restarts it on failure |
...
To keep the streaming job alive across crashes and reboots, install the systemd unit:

```bash
sudo cp infra/spark-streaming.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now spark-streaming
sudo systemctl status spark-streaming
