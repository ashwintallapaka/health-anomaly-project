...
wearable (simulated) → MQTT → Kafka → Spark (detection_rules.py) → MongoDB → dashboard
                                                       ↑
    data/historical_vitals.csv → Spark batch job (thresholds + personal-baseline z-scores) → per-patient summary
```
...
| `spark/batch_processing.py` | Batch job: reads historical vitals from CSV, flags anomalies two ways (the fixed thresholds, and a personal-baseline z-score check per patient), and writes a per-patient summary |
| `spark/test_detection_rules.py`, `spark/spark_test.py`, `spark/check_version.py` | Local smoke tests / cluster version check |
| `data/historical_vitals.csv` | Sample historical readings (3 patients, 45 rows — 15 each) that `batch_processing.py` runs against |
...
this reads a CSV of past readings and checks each one two ways — the same fixed
thresholds as `detection_rules.py`, and a personal-baseline z-score check (each
patient's own mean and standard deviation from this CSV, flagging anything more
than 3 standard deviations away). The per-patient summary it writes out includes
reading count, avg/min/max heart rate, avg/max temp, avg/min SpO₂, and separate
threshold-anomaly and baseline-anomaly counts — no Kafka, MongoDB, or broker needed.
...
(scatter chart, rule cards, and a paginated, filterable table — 10 rows per page)
run against 54 synthetic readings baked into the page, and a **historical batch
summary** table — the per-patient output of `spark/batch_processing.py` over
`data/historical_vitals.csv` (45 readings across 3 patients), including separate
threshold-anomaly and z-score-anomaly counts per patient — appended below it. Both
sections currently embed their data directly in the HTML rather than fetching it
...
- **Batch processing now does two independent anomaly checks**:
  `spark/batch_processing.py` reads `data/historical_vitals.csv`, checks each
  reading against the fixed thresholds (same as `detection_rules.py`) and
  against a personal-baseline z-score (each patient's own mean/stddev,
  flagging `|z| > 3`), then writes a per-patient summary — reading count,
  avg/min/max HR, avg/max temp, avg/min SpO₂, and separate threshold-anomaly
  and baseline-anomaly counts — to a CSV output. It's a separate, standalone
  job — it doesn't read from or write to Kafka or MongoDB, and isn't run
  automatically by anything else in the repo.
- **`docs/index.html` isn't actually wired to MongoDB or the batch job.**
  Both the rule-engine view and the historical batch summary table on the
  ...
  regenerated by hand — it doesn't fetch either source live.
  `docs/dashboard_data_contract.md` documents the shape for whoever picks up
  making that live.
- **Field and topic names still don't match `common/schemas.py`.** The live
...
- **The live streaming path still only knows fixed thresholds.** Mongo
  `alerts` documents are always `rule: "threshold"` — `streaming_job.py`
  hasn't picked up the personal-baseline z-score check that `batch_processing.py`
  now does. `ALERT_SCHEMA` anticipates `z_score`/`severity` fields for this;
  porting the batch job's baseline logic into the streaming job is the
  natural next step to close that gap.
...
- **Five branches are fully merged into `main` and safe to delete**:
  `Ashwin-spark-dashboard`, `abhay/day1`, `chhaya-batch-processing`,
  `chhaya-kafka-setup`, and `chhaya-personal-baselines`. Nothing is lost by
  removing them from GitHub's Branches page — `git log main..origin/<branch>`
  shows no unmerged commits on any of them as of this writing.
- **`paho-mqtt` is declared in two `requirements.txt` files** with
  different pins: `mqtt/requirements.txt` has `paho-mqtt==2.1.0`, while
  `kafka/requirements.txt` has an unpinned `paho-mqtt` (needed there because
  `mqtt_to_kafka_bridge.py` lives in `kafka/` but talks to both brokers). Not
  broken, just worth pinning consistently or moving the bridge's
  requirements somewhere less ambiguous.
